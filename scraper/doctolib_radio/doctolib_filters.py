...
# Override Doctolib catégories
DOCTOLIB_FILTERS = {'motives': {1233295: 'Echo des parties molles', 1233306 : 'Echo musculaire', }}
DOCTOLIB_CATEGORY = ["ECHOGRAPHIE"]
DOCTOLIB_CATEGORY = [c.lower().strip() for c in DOCTOLIB_CATEGORY]

# Fir debug : use an existing appointment
DOCTOLIB_FILTERS = {'motives': {1233455: 'Radio bassin'}}
DOCTOLIB_CATEGORY = ['RADIOGRAPHIES']
DOCTOLIB_CATEGORY = [c.lower().strip() for c in DOCTOLIB_CATEGORY]

def set_config(config_type="echo"):
    global DOCTOLIB_FILTERS, DOCTOLIB_CATEGORY
    if config_type == "echo":
        # Override Doctolib catégories
        DOCTOLIB_FILTERS = {'motives': {1233295: 'Echo des parties molles', 1233306 : 'Echo musculaire', }}
        DOCTOLIB_CATEGORY = ["ECHOGRAPHIE"]
        DOCTOLIB_CATEGORY = [c.lower().strip() for c in DOCTOLIB_CATEGORY]
    elif config_type == "radio":  
        # Fir debug : use an existing appointment
        DOCTOLIB_FILTERS = {'motives': {1233455: 'Radio bassin'}}
        DOCTOLIB_CATEGORY = ['RADIOGRAPHIES']
        DOCTOLIB_CATEGORY = [c.lower().strip() for c in DOCTOLIB_CATEGORY]
